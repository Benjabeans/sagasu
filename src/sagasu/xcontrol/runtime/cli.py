...
from sagasu.xcontrol.protocol import SagasuError, success, write_json
from sagasu.xcontrol.runtime import guard
from sagasu.xcontrol.runtime.capture import stream_png
from sagasu.xcontrol.runtime.cursor import create_backend, normalize_button
...
from sagasu.xcontrol.runtime.lock import LOCK_PATH, session_lock
...
def build_parser() -> argparse.ArgumentParser:
    parser = ProtocolArgumentParser(prog="sagasu-xcontrol")
...
def execute(
    ...
    *,
    ...
    pause_path: Path | str = guard.PAUSE_PATH,
) -> None:
    ...
    if arguments.command == "human":
        with session_lock(exclusive=True, path=lock_path):
            if arguments.human_command == "pause":
                guard.pause(pause_path)
                operation = "human.pause"
            else:
                guard.resume(pause_path)
...
def _execute_mutation(
    ...
    *,
    ...
    with session_lock(exclusive=True, path=lock_path):
        guard.require_agent_control(pause_path)
        display = get_display_size()
        current = (
            get_pointer_position()
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        error = SagasuError(
            "input_failed",
            "The X-control executor failed unexpectedly",
            {"reason": str(exc)},
        )
        write_json(error.as_dict(), sys.stderr)
        return error.exit_status
