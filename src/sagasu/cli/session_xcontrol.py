"""Host-side handlers for ``sagasu session`` X-control commands."""
...
from sagasu.xcontrol.host import SessionXControl
...
def run(arguments: argparse.Namespace, docker: DockerCLI) -> dict[str, Any]:
    ...
    control = SessionXControl(docker)
    if arguments.session_command == "screenshot":
    ...
    return control.invoke(resolved, runtime_arguments)
