...
from sagasu.protocol import SagasuError, parse_json_object
...
SESSION_EXECUTOR = "sagasu-session-exec"
...
class DockerCLI:
    ...
    def exec_json(
        ...
        payload = parse_json_object(
            completed.stdout, source="sagasu-session-exec"
        )
    ...
    def exec_stream(
        ...
        self._exec_stream(
            ...
            failure_message="The in-container screenshot command failed",
        )
    ...
    def exec_stream_json(
        ...
        if payload is None:
            raise SagasuError(
                "invalid_response",
                "sagasu-session-exec omitted streaming command metadata",
            )
        return payload
...
def _executor_result(stderr: bytes) -> dict[str, Any] | None:
    """Extract the executor's final successful JSON metadata line."""
